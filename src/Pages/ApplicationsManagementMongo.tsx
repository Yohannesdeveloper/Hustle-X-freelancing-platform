  ...
  ({
    ...
    return (
      <motion.div
        ...
        exit={{ opacity: 0, y: -20 }}
        className={`${
          darkMode ? "bg-black/50 border-white/10" : "bg-white border-black/10"
              ...
              {application.cvUrl && (
                <motion.button
                  onClick={() =>
                    handleDownloadCV(
                      application.cvUrl,
                      `${application.applicant.profile?.firstName ?? ""}_${
                        application.applicant.profile?.lastName ?? ""
                      }`
                    )
                  }
  ...
  const handleStatusUpdate = useCallback(
    ...
    setUpdating(prev => {
      if (prev[applicationId]) {
        ...
        return prev; // Already updating, skip
      }
      console.log(`Starting update for application ${applicationId}`);
      return { ...prev, [applicationId]: true };
    });

    // Use a timeout to ensure state has updated before proceeding
    setTimeout(() => {
      setUpdating(current => {
        if (!current[applicationId]) {
          console.log(`Application ${applicationId} is not in updating state, skipping`);
          return current; // Not updating, skip
        }
  ...
  const performStatusUpdate = useCallback(
    ...
    try {
      ...
      const statusMessages: Record<Application["status"], string> = {
        ...
        hired: `We are pleased to inform you that your application at ${
          application.company || "the company"
        } has been successful, and you have been selected for the position.`,
        rejected: `We regret to inform you that your application at ${
          application.company || "the company"
      ...
      const combinedMessage = combinedSuccess
        ...
        : `${statusActionMessages[newStatus]} ${
            emailErrorMessage
              ? emailErrorMessage + " "
              : smsErrorMessage
              ? smsErrorMessage
            ...
            {tabs.map((tab, index) => {
              ...
              return (
                <motion.button
                  ...
                  className={`flex items-center gap-2 px-3 sm:px-4 py-2 sm:py-3 rounded-lg sm:rounded-xl font-medium transition-all duration-300 shadow-md font-inter tracking-tight text-sm sm:text-base ${
                    activeTab === tab.id
                      ? darkMode
